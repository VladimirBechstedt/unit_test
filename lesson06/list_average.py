...
class AverageGenerator:
    ...
    def find_average_size_error(self):
        if len(self.new_list) < 2:
            raise ValueError("Размер списка менее 2 элементов.")

    def __str__(self):
        return f'Среднее арифметическое списка {self.new_list} равно {self.average}'
...
class AverageComparator:
    ...
    def avg_compare(first_avg, second_avg):
        if first_avg > second_avg:
            return 'Первое среднеарифметическое значение списка больше, чем второе.'
        elif first_avg < second_avg:
            return 'Второе среднеарифметическое значение списка больше, чем первое.'
        else:
            return 'Среднеарифметическое значение списков равно.'